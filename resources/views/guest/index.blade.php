    ...
    <main class="pt-16 bg-gradient-to-br from-amber-50 via-orange-50 to-yellow-100 min-h-screen relative overflow-hidden" style="zoom: 80%;">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 lg:gap-12">
                @foreach($produks as $index => $p)
                    @php
                        ...
                        // Determine route based on authentication and role
                        if (!$user) {
                            // If not authenticated, redirect to login
                            $route = route('login');
                        } elseif ($user->role === 'admin') {
                            // If admin, go to edit page
                            $route = route('produk.edit', $p->id);
                        } else {
                            // If regular user, go to product detail
                            $route = route('produk.show', $p->id);
                        }
                    @endphp
                        ...
                        <div class="bg-white/80 backdrop-blur-lg rounded-3xl overflow-hidden shadow-2xl hover:shadow-amber-500/20 border border-white/50 relative">
                            ...
                            <a href="{{ $route }}" class="block relative overflow-hidden">
                                <div class="h-80 relative">
                                    ...
                                    <!-- Login Required Badge (only shown for non-authenticated users) -->
                                    @if(!auth()->user())
        ...
        <div class="relative z-10 bg-gradient-to-r from-amber-600 via-orange-600 to-yellow-600 py-16 mt-16">
            <div class="max-w-4xl mx-auto text-center px-6">
                ...
                @if(auth()->user())
                    <a href="#" class="inline-flex items-center px-8 py-4 bg-white text-amber-600 font-bold rounded-full shadow-2xl hover:shadow-white/20 transform hover:scale-105 transition-all duration-300">
                ...
                @else
                    <a href="{{ route('login') }}" class="inline-flex items-center px-8 py-4 bg-white text-amber-600 font-bold rounded-full shadow-2xl hover:shadow-white/20 transform hover:scale-105 transition-all duration-300">
                        <svg class="w-6 h-6 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 16l-4-4m0 0l4-4m-4 4h14m-5 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013 3v1"/>
                        </svg>
                        Login untuk Memesan
