<x-app-layout>
    <main
        class="pt-16 bg-gradient-to-br from-amber-50 via-orange-50 to-yellow-100 min-h-screen relative overflow-hidden"
        style="zoom: 80%;">
        <!-- Background Pattern -->
        <div class="absolute inset-0 opacity-10">
            <div class="absolute top-20 left-10 w-32 h-32 bg-amber-400 rounded-full blur-3xl animate-pulse"></div>
            <div
                class="absolute top-40 right-20 w-24 h-24 bg-orange-400 rounded-full blur-2xl animate-pulse delay-1000">
            </div>
            <div
                class="absolute bottom-32 left-1/4 w-40 h-40 bg-yellow-400 rounded-full blur-3xl animate-pulse delay-2000">
            </div>
        </div>
        ...
        <div class="relative z-10 text-center py-16 mb-12">
            <div class="max-w-4xl mx-auto px-6">
                <h1
                    class="text-5xl md:text-7xl font-bold bg-gradient-to-r from-amber-600 via-orange-600 to-yellow-600 bg-clip-text text-transparent mb-6 font-serif">
                    Menu Spesial
                </h1>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 lg:gap-12">
                @foreach($produks as $index => $p)
                    ...
                    <div class="group relative transform transition-all duration-500 hover:scale-105 hover:-translate-y-2"
                        ...
                        <!-- Product Card -->
                        <div
                            class="bg-white/80 backdrop-blur-lg rounded-3xl overflow-hidden shadow-2xl hover:shadow-amber-500/20 border border-white/50 relative">
                            ...
                            <div class="p-6 relative">
                                <!-- Title -->
                                <h3
                                    class="text-2xl font-bold text-gray-800 mb-3 font-serif group-hover:text-amber-600 transition-colors duration-300">
                                    {{ $p->nama }}
                                </h3>
                                ...
                                @if(auth()->user() && auth()->user()->role === 'admin')
                                    <div class="mt-6 pt-4 border-t border-gray-200 flex justify-between items-center">
                                        ...
                                        <form action="{{ route('produk.destroy', $p->id) }}" method="POST"
                                            onsubmit="return handleDelete(event, '{{ $p->nama_produk }}');" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                            ...
                            <!-- Decorative Corner -->
                            <div
                                class="absolute top-0 right-0 w-20 h-20 bg-gradient-to-bl from-amber-400/20 to-transparent rounded-bl-3xl">
                            </div>
                        </div>
                    </div>
                @endforeach
        ...
        <div class="relative z-10 bg-gradient-to-r from-amber-600 via-orange-600 to-yellow-600 py-16 mt-16">
            <div class="max-w-4xl mx-auto text-center px-6">
                ...
                <p
                    class="inline-flex items-center px-8 py-4 bg-white text-amber-600 font-bold rounded-full shadow-2xl hover:shadow-white/20 transform hover:scale-105 transition-all duration-300">
    ...
    <script>
        function handleDelete(event, productName) {
            event.preventDefault(); // Stop form submission

            Swal.fire({
                title: 'Konfirmasi Hapus',
                html: `Apakah Anda yakin ingin menghapus produk?`,
                icon: 'warning',
